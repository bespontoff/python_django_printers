...
class PrintersModel(models.Model):
    ...
    cartridges = models.ManyToManyField('CartridgesModel', blank=True, related_name='cartridges_printers',
                                        verbose_name='Модели картриджей')
    sn_oid = models.ForeignKey('SNMP_OIDModel', blank=True, null=True, on_delete=models.PROTECT, related_name='sn_oid',
                               verbose_name='S/N OID')
    printed_pages_all_oid = models.ForeignKey('SNMP_OIDModel', blank=True, null=True, on_delete=models.PROTECT,
                                              related_name='printed_pages_all_oid',
                                              verbose_name='Распечатано страниц (всего) OID')
...
class SNMP_OIDModel(models.Model):
    type_OID = models.ForeignKey('Type_OIDModel', blank=True, null=True, on_delete=models.PROTECT,
                                 related_name='type_OID', verbose_name='Type_OID')
...
class Printers_in_serviceModel(models.Model):
    service_object = models.ForeignKey('Service_objectModel', blank=True, null=True, on_delete=models.PROTECT,
                                       verbose_name='Объект_обслуживания')
    serial_number = models.CharField(max_length=50, verbose_name='S/N', db_index=True)
    printers = models.ForeignKey('PrintersModel', on_delete=models.PROTECT, verbose_name='Модель принтера',
                                 related_name='printers_fk')
    status_printer = models.ForeignKey('StatusPrintersModel', on_delete=models.PROTECT, verbose_name='Статус',
                                       related_name='status_printer_fk')
...
class Printers_in_service_commentsModel(models.Model):
    printers_in_service = models.ForeignKey('Printers_in_serviceModel', blank=True, null=True, on_delete=models.PROTECT,
                                            verbose_name='Принтер (учет)')
...
class Printed_pagesModel(models.Model):
    ...
    created = models.DateTimeField(auto_now_add=True,
                                   db_index=True)  # значение True приводит к тому, что поле становится невидимым и
    ...
    error_message = models.CharField(max_length=100, blank=True, null=True, verbose_name='Ошибки')
