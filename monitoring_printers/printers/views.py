...
def status_printers_list_view(request):
    ...
    return render(request, 'printers/status_printers_listview.html',
...
def status_printers_delete_view(request, id):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'printers/delete.html',
...
def print_servers_list_view(request):
    ...
    return render(request, 'printers/print_servers_listview.html',
                  {'form': form, 'dataset': dataset, 'title_text': title_text})
...
def cartridges_list_view(request):
    ...
    return render(request, 'printers/cartridges_listview.html',
                  {'form': form, 'dataset': dataset, 'title_text': title_text})
...
def printers_list_view(request):
    ...
    return render(request, 'printers/printers_listview.html',
                  {'form': form, 'dataset': dataset, 'title_text': title_text})
...
def typeOID_list_view(request):
    ...
    title_text = "Type_OID"
    return render(request, 'printers/typeOID_listview.html',
                  {'form': form, 'dataset': dataset, 'title_text': title_text})
...
def SNMP_OID_list_view(request):
    ...
    title_text = "SNMP_OID"
    return render(request, 'printers/SNMP_OID_listview.html',
                  {'form': form, 'dataset': dataset, 'title_text': title_text})
...
def printers_in_service_list_view(request):
    ...
    return render(request, 'printers/printers_in_service_listview.html',
                  {'form': form, 'dataset': dataset, 'title_text': title_text})
...
def printers_in_service_list_view_filter(request):
    ...
    # Получаем все записи
    dataset = Printers_in_serviceModel.objects.all().select_related('service_object', 'printers', 'status_printer',
                                                                    'print_server', ).exclude(archived=True)
...
def service_object_all_export_printed_pages_xls_bmk(request):
    ...
    rows = []
    # Получаем все неархивные записи из Printers_in_serviceModel с учетом объекта обслуживания
    dataset_printers_in_service = Printers_in_serviceModel.objects.filter(archived=False).exclude(
        service_object__service_object_name="ББ")
...
def async_service_object_printed_pages_list_view(request, id):
    ...
    for id_printer in id_printers_massive:
        try:
            ...
            data_printers_in_service = Printed_pagesModel.objects.filter(printers_in_service=id_printer).latest(
                'created')
            ...
            print(
                f"Ошибка выполнения функции async_service_object_printed_pages_list_view(request, id): {ex} ||| id_printer: {id_printer}")
            continue
